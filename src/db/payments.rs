...
async fn payment_amount_scoped(pool: &PgPool, company_id: Uuid, payment_id: Uuid) -> Result<rust_decimal::Decimal> {
    let amount = sqlx::query_scalar::<_, rust_decimal::Decimal>(
        "SELECT amount FROM payments WHERE id = $1 AND company_id = $2",
    )
    .bind(payment_id)
    .bind(company_id)
    .fetch_optional(pool)
...
async fn act_available_amount_for_payment(
    pool: &PgPool,
    company_id: Uuid,
    payment_id: Uuid,
    act_id: Uuid,
...
async fn invoice_available_amount_for_payment(
    pool: &PgPool,
    company_id: Uuid,
    payment_id: Uuid,
    invoice_id: Uuid,
...
pub async fn reconcile_split_scoped(
    ...
    let payment_amount = payment_amount_scoped(pool, company_id, payment_id).await?;
    anyhow::ensure!(
    ...
    for allocation in allocations {
        match allocation.document_kind.as_str() {
            "act" => {
                ensure_act_in_company(pool, company_id, allocation.document_id).await?;
                let available =
                    act_available_amount_for_payment(pool, company_id, payment_id, allocation.document_id).await?;
                anyhow::ensure!(
            ...
            "invoice" => {
                ensure_invoice_in_company(pool, company_id, allocation.document_id).await?;
                let available = invoice_available_amount_for_payment(
                    pool,
                    company_id,
                    payment_id,
