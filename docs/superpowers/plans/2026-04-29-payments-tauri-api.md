...
**Architecture:** All business logic lives in `src/tauri_api/payments.rs`. Helper functions (parsing, formatting, bank-import pipeline) are private to that file — extracted from `src/ui/payments.rs` where they already exist. The Tauri command layer in `src-tauri/src/commands/payments.rs` is already wired and must not change.

**Tech Stack:** Rust async/await, sqlx (runtime-style), tokio::join!, rust_decimal, uuid, chrono, import::bank_csv parsers.

---

## DTO Shape Contract

Before writing code, confirm exact field mapping between Rust DTOs and TypeScript types:

| Rust `PaymentItemDto` | TS `PaymentItemDto` | Notes |
|---|---|---|
| `id: String` | `id: string` | UUID.to_string() |
| `date: String` | `date: string` | DD.MM.YYYY from DB |
| `counterparty: String` | `counterparty: string` | name or "" |
| `amount_str: String` | `amountStr: string` | formatted UA money |
| `direction: String` | `direction: "in" \| "out"` | Income→"in", Expense→"out" |
| `matched_doc: String` | `matchedDoc: string` | "" for now (no join) |
| `account: String` | `account: string` | bank_name or "" |

| Rust `PaymentsKpiDto` | TS `PaymentsKpiDto` | Notes |
|---|---|---|
| `incoming_str: String` | `incomingStr: string` | formatted |
| `outgoing_str: String` | `outgoingStr: string` | formatted |
| `net_str: String` | `netStr: string` | incoming − outgoing |
| `unmatched_str: String` | `unmatchedStr: string` | count as string |
| `incoming_sub: String` | `incomingSub: string` | "поточний місяць" |
| `outgoing_sub: String` | `outgoingSub: string` | "поточний місяць" |
| `unmatched_count: i32` | `unmatchedCount: number` | raw i64→i32 |

`PaymentCreateOrUpdateRequest` (Rust, camelCase via serde) ↔ `PaymentDraftFormDto` (TS) — field names match after serde rename.
...
| File | Action |
|---|---|
| `src/tauri_api/payments.rs` | **Rewrite** — add private helpers + implement all 7 stubs |
| `src-tauri/src/commands/payments.rs` | No change |
| `frontend/src/lib/types.ts` | No change |
| `frontend/src/lib/api.ts` | No change |

---

## Task 1: Rewrite `src/tauri_api/payments.rs` with helpers + all implementations
...
pub async fn payment_create_or_update(
    ...
    if let Some(id) = parse_optional_payment_id(&request.id)? {
        ...
        .await?
        .ok_or_else(|| {
            anyhow!("Платіж не знайдено або він не належить поточній компанії")
        })?;
        Ok(MutationResultDto {
            ok: true,
...
pub async fn payment_unreconcile(
    ...
    let id = Uuid::parse_str(&payment_id)
        .map_err(|_| anyhow!("Невалідний ідентифікатор платежу"))?;
    let changed =
        db::payments::mark_unreconciled_scoped(ctx.pool(), ctx.company_id(), id).await?;
    if !changed {
        return Err(anyhow!(
            "Платіж не знайдено або він не належить поточній компанії"
        ));
...
- Missing `open` crate → check `Cargo.toml` for `open = "..."` dependency (already used in `ui/payments.rs`)
...
git commit -m "feat(tauri-api): implement payments backend — list, create/update, reconcile, import CSV, open template"
```

---

## Remaining TODOs (explicit, with blockers)

These are intentional gaps — not bugs, but known limitations to address later:

| Field | Current | Blocker |
|---|---|---|
| `PaymentItemDto.matched_doc` | Always `""` | `db::payments::list` query does not join `payment_acts`/`payment_invoices`; needs a separate query or JOIN |
| `payments_sync_bank` | Identical to import CSV | Real bank sync would need API credentials per bank; out of scope until bank integrations are defined |
| `PaymentsKpiDto.unmatched_str` | Raw count as string | If UI expects a formatted amount, need a separate unmatched-sum query |

---

## Self-Review Checklist

- [x] All 7 stub functions implemented (payments_list, payment_create_or_update, payment_reconcile, payment_unreconcile, payments_import_latest_csv, payments_sync_bank, payments_open_manual_template)
- [x] Decimal used for all amounts — never f32/f64
- [x] NaiveDate used for dates
- [x] company-scoped DB operations (list, update_scoped, mark_reconciled_scoped, mark_unreconciled_scoped)
- [x] tokio::join! for parallel queries in payments_list
- [x] direction mapped Income→"in", Expense→"out" to match TS `"in" | "out"`
- [x] All serde field names match TS (camelCase via rename_all)
- [x] No unwrap() in production paths
- [x] Unit tests cover all pure helper functions
